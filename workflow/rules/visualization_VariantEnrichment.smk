...
rule gather_metric_ranges:
	...
	conda:
		os.path.join(ENV_DIR, "GWAS_env.yml")
	script:
		os.path.join(SCRIPTS_DIR, "visualization", "gather_metric_ranges_VariantEnrichment.R")
...
rule plot_thresholded_performance_comparison:
	...
	conda:
		os.path.join(ENV_DIR, "GWAS_env.yml")
	script:
		os.path.join(SCRIPTS_DIR, "visualization", "plot_thresholded_performance_comparison_VariantEnrichment.R")