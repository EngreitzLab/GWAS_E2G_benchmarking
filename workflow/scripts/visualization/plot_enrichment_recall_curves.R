...
add_plotting_params <- function(df, cp, p_threshold) {
	df = dplyr::filter(df, recall>0.005, p_adjust_enr<p_threshold, !is.na(enrichment),  # remove 0,0 points ( and arbitrarily low recall), non-significant and NA enrichments
		...
		left_join(cp, by=c("method")) # add hex and pred_name_long and score_threshold
...
plot_single_panel <- function(df, key_this, pred_colors, all_var, trait_names, trait_groups, df_sep) {
	# organize
	...
	print(key_this)
	print(summary(df))
	#n_label = get_n_equals(df, "variant_overlap") df, var, traits, trait_groups)

	if (key_this == "all_matched.all_matched") {
		n_label = get_n_equals_variant_overlap(df_sep, all_var, trait_names, trait_groups) 
	} else {
		n_label = get_n_equals_variant_overlap(df, all_var, trait_names, trait_groups) 
	}
	...
	g=ggplot(data=df_plot, aes(x=recall, y=enrichment, color=pred_name_long)) +
		...
		geom_point(data=df_binary, aes(x=recall, y=enrichment, color=pred_name_long), size=3) +
		geom_point(data=df_thresh, aes(x=recall, y=enrichment, color=pred_name_long), size=3) +
...
trait_groups = fread(snakemake@input$traitGroups)
method_names = snakemake@params$method_names %>% strsplit(" ") %>% unlist()
...
# aggregate plot 
a = plot_single_panel(df = df_agg, key_this = "all_matched.all_matched", pred_colors = pred_colors, all_var = all_var, trait_names = unique(trait_names), 
	trait_groups = trait_groups, df_sep = df_sep)
...
for (i in 1:nrow(map)){
	...
	plot_list[[i]] = plot_single_panel(df_sep, map$key[i], pred_colors, all_var, c(map$trait[i]), trait_groups, df_sep) + ggtitle(title) + theme(legend.position="None")
}
